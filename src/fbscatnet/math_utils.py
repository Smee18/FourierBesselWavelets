...
def _first_kind_bessel(X, order : int) -> np.ndarray:
    """Vectorized Bessel function of the first kind supporting negative orders."""
    # scipy.special.jv natively handles array inputs and complex/float arguments efficiently
    if order < 0:
        return (-1) ** abs(order) * scipy.special.jv(abs(order), X)
    return scipy.special.jv(order, X)

def _first_kind_bessel_deriv(X, order):
...
def _first_modified_bessel(X, order):
    """Vectorized modified Bessel function of the first kind."""
    # scipy.special.iv computes I_v(x) natively and fast
...
def _generate_fourier_bessel_wavelet(m, k, size=50, sigma=0.1, freq_limit=20, verbose=False):
    ...
    if verbose:
        ...
        print(f"Wavelet  (m={m}, k={k}) | Mean (~0): {mean_val:.2e} | L2: {post_norm_energy:.4f} Eigenvalue: {eigenvalue:.4f}")
    # -------------------------
