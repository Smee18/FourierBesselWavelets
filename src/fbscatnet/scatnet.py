...
class FourierBesselScatNet():
    ...
    def __init__(self, size, bank):
        ...
        self.low_pass = bank.get_wavelet(0,0)
    ...
    def generate_embeddings(self, data, downsize, batch_size = 32):
        ...
        final_features = np.zeros((num_samples, (d_size * d_size * self.num_filters)), dtype=np.float32)
        ...
        for start in tqdm(range(0, num_samples, batch_size), desc="Processing Batches"):
            ...
            batch_pooled = np.zeros((end - start, d_size, d_size, self.num_filters), dtype=np.float32)
            ...
            for i, key in enumerate(self.bank_keys[1:]):
                ...
                batch_pooled[..., i] = (filtered_spatial.reshape(-1, d_size, downsize, d_size, downsize).mean(axis=(2, 4)))
